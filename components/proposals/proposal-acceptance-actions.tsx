...
export default function ProposalAcceptanceActions({ token }: { token: string }) {
      ...
      <div className="flex items-start gap-3.5 rounded-[18px] border border-emerald-200 bg-emerald-50 px-6 py-5">
        ...
        <div>
          ...
          <p className="mt-0.5 text-[0.83rem] text-emerald-700">
            Thank you for accepting. Our team will contact you shortly to progress your order.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex items-start gap-3.5 rounded-[18px] border border-gray-200 bg-gray-50 px-6 py-5">
        ...
        <div>
          ...
          <p className="mt-0.5 text-[0.83rem] text-gray-600">
            You have declined this proposal. Okelcor will be in touch if you&apos;d like to revisit.
          </p>
        </div>
      </div>
    );
  ...
  const handleAccept = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/proposals/${encodeURIComponent(token)}/accept`, {
        method: "POST",
      });
      if (res.ok) {
        setStatus("accepted");
  ...
  const handleReject = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/proposals/${encodeURIComponent(token)}/reject`, {
        method: "POST",
      ...
      {!showReject ? (
        <div className="flex flex-wrap gap-3">
          ...
          >
            {loading === "accept"
              ? "Accepting…"
              : <><CheckCircle2 size={14} strokeWidth={2} /> Accept Proposal <ArrowRight size={14} strokeWidth={2.5} /></>
            }
          </button>
          <button
            type="button"
