/**
 * Admin tracking — ordered route points for playback.
 * GET → GET /admin/tracking/devices/{id}/route?from=&to=   (defaults last 24h)
 */
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${BASE}/tracking/devices/${id}/route${query ? `?${query}` : ""}`, {
